...
- **In-order, un-split per-producer presentation.** For any single producer+partition, the engine presents batches to `commit` in send order, and never splits one producer's batches across two concurrent in-flight `commit` calls. (Else idempotent sequence-gap detection and epoch fencing misfire.) Satisfied by a single flush worker or by sharding commit concurrency on the producer identity — never by partition-sorting the buffer.
